...
{#snippet sidebarLegend()}
	<div
		class="menu w-full rounded-box shadow-lg bg-neutral-900 border border-neutral-700 opacity-90 p-2"
	>
		<div class="menu-title flex flex-row justify-between items-center px-2 py-2">
			...
			{#each allGroupedPosts as group}
				<li>
					...
					>
						...
						<span class="badge badge-sm badge-neutral">{group.months.reduce((acc, m) => acc + m.posts.length, 0)}</span>
					</button>
							...
							{#each group.months as monthGroup}
								<li>
									...
									>
										...
										<span class="badge badge-xs badge-ghost">{monthGroup.posts.length}</span>
									</button>
								</li>
							{/each}
						</ul>
	...
	<Container>
		...
		<!-- Sidebar Legend (Visible only on large screens) -->
			<div class="hidden flex-none lg:block lg:w-64">
				<div
					class="sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto scrollbar-thin scrollbar-thumb-base-300 scrollbar-track-base-100"
				>
					{@render sidebarLegend()}
				</div>
			</div>
